...
export const useTenantAppointments = () => {
  ...
  const fetchTenantAppointments = async () => {
    try {
      setLoading(true)
      const { list } = await lumi.entities.tenant_appointments.list({
        where: { tenantId: tenant._id },
        orderBy: { appointmentDate: 'desc' }
      })
      setAppointments(list || [])
    } catch (err) {
      console.error('Erro ao buscar agendamentos do tenant:', err)
  ...
  const createAppointment = async (appointmentData) => {
    try {
      ...
      const newAppointment = await lumi.entities.tenant_appointments.create({
        ...appointmentData,
        tenantId: tenant._id,
  ...
  const updateAppointment = async (appointmentId, updates) => {
    try {
      const updatedAppointment = await lumi.entities.tenant_appointments.update(appointmentId, {
        ...updates,
        updatedAt: new Date().toISOString()
      })
  ...
  const deleteAppointment = async (appointmentId) => {
    try {
      await lumi.entities.tenant_appointments.delete(appointmentId)
  ...
  const getAvailableTimeSlots = (date, serviceId, serviceDuration) => {
    // Horários de funcionamento baseados nas configurações do tenant
    const workingHours = tenant?.businessHours || {
      start: 9,
      end: 18
    }
  ...
  return {
    ...
    refetch: fetchTenantAppointments
  }
}
